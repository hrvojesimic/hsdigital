...
*Hrvoje Šimić, 2020-10-26 — data from [Our World in Data](https://ourworldindata.org/coronavirus-source-data)*

If we plot COVID-19 daily reported cases and deaths accross all EU countries, we get this:
...
Let's take a simple symetric 31-day average to get rid of the noise:

<figure style="max-width: 500px; margin: auto">
  <div class="covidChart" data-show="cd" data-territory="EU" data-offset="0" data-dmax="21.5" data-window="31"></div>
  <figcaption>Plot of daily reported cases and deaths in EU, averaged accross 31 days. Last 31 days are progressively faded as they could change with future data.</figcaption>
</figure>
...
Does this wave of reported cases emerge in every European country? A systematic approach would be to formaly define parameters of the first wave, the one that occurred in the first six months of 2020. On the 31-day average time series we are using these criteria:
...
When there is no averaging you can notice small waves that are there because of the weekly variation of data. These disappear when we calculate simple 7-day moving average.

31-day window moving agerage is quite similar to 7-day. The correlation coefficient is somewhat higher because some of the noise gets averaged out.
...
Serbia here is instructive, as it shows some correlation around 12 days in the no averaging condition, but this fades out with wider averaging windows. United Kindgom's peak correlation is stuck at zero, suggesting very late testing or post-mortem testing in the course of disease in the initial phase of the epidemic.
...
Differences are striking, but the map is not in chaos. There are noticable geographical patterns:
...
- Central Europe countries mostly have well-defined first wave curves, with offset at around two weeks. Norway and Iceland follow suit, while Denmark doesn'tm and Sweden is an obvious black sheep in the European flock thanks to its epidemiology policies.
